...
public class HIBCRoute extends BaseRoute {
	...
	public void configure() throws Exception {
		...
		from(DIRECT_HIBC_HTTP).routeId("hibc-http-route")
     		...
     		.log(LoggingLevel.INFO, "TransactionId: ${exchange.exchangeId}, Preparing message for HIBC.")
			...
			.when(isEnrollment())
				...
				.log(LoggingLevel.INFO, "TransactionId: ${exchange.exchangeId}, Sending to Enrollment endpoint")
				.to(enrollmentHttpUrl).id("ToHibcEnrollment")
            .otherwise()
            	.log(LoggingLevel.INFO, "TransactionId: ${exchange.exchangeId}, Found unexpected message of type: ${exchangeProperty.messageType}")
            .end()
	        .log(LoggingLevel.INFO, "TransactionId: ${exchange.exchangeId}, Received response from HIBC.")
            .to("log:HttpLogger?level=DEBUG&showBody=true&showHeaders=true&multiline=true")
