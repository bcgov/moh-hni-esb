 ...
 * format:{messageid}-{messagetype}-{facilityid}-{messagedate}-response.txt
 * This bean will be called onCompletion of main route.
 * When we use onCompletion, Camel spin-off a copy of the Exchange and routes it in a separate thread, kinda like a Wire Tap.
...
public class ResponseFileDropGenerater extends FileDropGenerater {
	...
	public void createFileDrops(Exchange exchange) {
		...
		writeFiledrop(exchange, responseFileName);
		logger.info("{} - TransactionId: {}, Successfully created file drops for response: {}",LoggingUtil.getMethodName(), exchange.getProperty(Exchange.CORRELATION_ID, String.class), responseFileName);	        
	}
