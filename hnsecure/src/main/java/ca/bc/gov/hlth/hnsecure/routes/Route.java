...
import ca.bc.gov.hlth.hnsecure.parsing.DateTimeRequestProcessor;
...
public class Route extends BaseRoute {
    ...
    public void configure() {
        ...
        from("jetty:http://{{hostname}}:{{port}}/{{endpoint}}?httpMethodRestrict=POST").routeId("hnsecure-route")
        	...
        	.log("HNSecure received a request: ${body}")
        	...
        	// Prevent file drop and auditing for receiving app HNETDTTN.
        	.process(new DateTimeRequestProcessor()).id("DTRequest")
	...
	/**
	 * This method performs the steps required before configuring the route 1. Set
	 * the MQ configuration 2. Set the transaction id generator for messages 3. Load
	 * application properties 4. Initializes the validator classes
	 */
	private void init() {
	...
	/**
	 * This method is used to concat multiple Predicates for HIBC message types. It
	 * builds a compound predicate to be used in the Route.
	 */
	...
	/**
	 * This method injects application properties set in the context to
	 * ApplicationProperties class This helps in using the properties across the
	 * application without extending classes as RouteBuilder. Sample test code to
	 * use application properties: RouteTest.testApplicationPropertiesLoader()
	 */
	...
	/**
	 * This method initializes the Validators. If there was an error in loading
	 * validators, HNSecure server will not start Currently only TokenValidator is
	 * throwing error.
	 */
	public void loadValidator() {
		try {
			// This syntax will make sure validation is happening in the order of First
			// Token validation then Payload validation then ValidatorImpl
	...
	/**
	 * Creates MQ connection and sets it on a JMS Component which is added to the
	 * camel context.
	 */
	private void initMQ() {
	...
	private MQQueueConnectionFactory mqQueueConnectionFactory() {
		...
		try {
			...
			logger.error("{} - MQ connection factory initialization failed with the error : {}", methodName,
					je.getMessage());
	...
	private void initFileDrop() {
		...
		if (isFileDropsEnabled) {
			...
			if (!fileDropsDirectory.exists()) {
				logger.debug("{} - File drop location directory [{}] did not exist so it will be created.", methodName,
						fileDropsLocation);
