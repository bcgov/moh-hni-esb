...
DROP INDEX IF EXISTS IXFX_TRANSACTION_INFO_TRANSACTION;
CREATE INDEX IXFX_TRANSACTION_INFO_TRANSACTION ON hnsecure.TRANSACTION_INFO (TRANSACTION_ID);
...
CREATE INDEX transaction_event ON hnsecure.TRANSACTION_EVENT (TRANSACTION_ID);
...
CREATE TABLE hnsecure.EVENT_MESSAGE(
	...
	ERROR_LEVEL	VARCHAR(255),affected_party
	MESSAGE_TEXT	TEXT,
	TRANSACTION_EVENT_ID	BIGINT
);

DROP INDEX IF EXISTS transaction_event"transaction"transaction_eventIXFX_EVENT_MESSAGE_TRANSACTION_EVENT;
